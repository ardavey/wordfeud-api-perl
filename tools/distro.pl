...
foreach my $lang ( sort keys $dist ) {
  say "$dist->{$lang}{name}";
  say '-' x length $dist->{$lang}{name};
  
  my %dist;
  foreach my $t ( @{ $dist->{$lang}{tileset} } ) {
    $dist{$t}++;
  }
  my $c = 1;
  print "   ";
  foreach my $t ( sort keys %dist ) {
   # print "$t x $dist{$t} ($dist->{$lang}{points}{$t} pt)\t";
    printf "%-2s x %-5s", $t, $dist{$t};
    print "\n   " unless ( $c++ % 6 );
  }
